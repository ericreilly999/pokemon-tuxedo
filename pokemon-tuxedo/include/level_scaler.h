...
/* Player party average for rival scaling (Req 22) */
u8 GetPlayerTop3Average(void);

/* Rival level calculation (Req 22: role-slot detection).
   is_champion_slot = TRUE  -> Champion formula: wild_max + 15.
   is_champion_slot = FALSE -> Rival formula: player top-3 average (floor 5). */
