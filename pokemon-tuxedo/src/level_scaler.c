...
/**
 * Get the average level of the player's top 3 non-egg party members.
 *
 * Reads gPlayerParty, excludes empty slots and eggs, sorts the
 * levels descending, and returns the floor average of the top 3
 * (or fewer if less than 3 eligible members exist).
 *
 * Returns 1 if no eligible party members are found.
 *
 * Validates: Requirement 22 (rival slot detection)
 */
...
/**
 * Calculate rival average Pokemon level (Req 22).
 *
 * Two formulas depending on is_champion_slot:
 * - Champion slot: wild_max + 15 (clamped 1-100)
 * - Rival slot: player top-3 party average (floor 5)
 *
 * @param badge_count Current badge count
 * @param region_id Current region ID
 * @param is_champion_slot TRUE if this rival occupies the Champion trainer class
 */
...
/**
 * Calculate rival ace Pokemon level (Req 22).
 * Formula: GetRivalAverageLevel(...) + 4, clamped to 100.
 *
 * @param badge_count Current badge count
 * @param region_id Current region ID
 * @param is_champion_slot TRUE if this rival occupies the Champion trainer class
...
/**
 * Get the current level cap (Tuxedo multi-region version).
 * Renamed to avoid collision with pokeemerald's GetCurrentLevelCap in caps.c.
 */
u8 GetTuxedoLevelCap(void)
{
    ...
    total_badges = GetTotalBadgeCount();
