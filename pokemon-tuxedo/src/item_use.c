...
/* MAP_NEW_BARK_TOWN placeholder until Johto maps are integrated (Phase 5) */
...
/* ============================================================ */
/* Pokemon Tuxedo: Region Ticket Handlers (Requirement 24)       */
/* ============================================================ */
...
/* Callback for Hoenn ticket warp executed after bag menu closes */
static void ItemUseOnFieldCB_HoennTicket(u8 taskId)
{
    struct RegionWarpData warp_data;
    /* Execute region transition (updates Region_Manager state) */
    TransitionToRegion(REGION_HOENN);
    /* Register Hoenn starting town as a fly destination (Req 24.3) */
    RegisterFlyLocationForRegion(MAP_LITTLEROOT_TOWN, REGION_HOENN);
...
static void ItemUseOnFieldCB_JohtoTicket(u8 taskId)
{
    ...
    /* Register Johto starting town as a fly destination (Req 24.3) */
    RegisterFlyLocationForRegion(MAP_NEW_BARK_TOWN, REGION_JOHTO);
    /* Get the starting location for Johto */
    GetRegionStartLocation(REGION_JOHTO, &warp_data);
    /* Execute the warp (will warp to MAP_UNDEFINED until Johto maps are integrated) */
    SetWarpDestination(warp_data.mapGroup, warp_data.mapNum, warp_data.warpId, warp_data.x, warp_data.y);
    DoWarp();
    ResetInitialPlayerAvatarState();
    DestroyTask(taskId);
 ...
 * Ticket is NOT consumed after use (Req 24.2).
 *
 * Validates: Requirement 24, AC 1-9
 */
void ItemUseOutOfBattle_HoennTicket(u8 taskId)
{
    ...
    {
        if (!gTasks[taskId].tUsingRegisteredKeyItem)
            DisplayItemMessage(taskId, FONT_NORMAL, sText_HoennRegionLocked, CloseItemMessage);
        else
            DisplayItemMessageOnField(taskId, sText_HoennRegionLocked, Task_CloseCantUseKeyItemMessage);
    }
}
 ...
 * Ticket is NOT consumed after use (Req 24.2).
 *
 * Note: Johto maps not yet integrated. Warp target is MAP_UNDEFINED until Phase 5.
 *
 * Validates: Requirement 24, AC 1-9
 */
void ItemUseOutOfBattle_JohtoTicket(u8 taskId)
{
    ...
    {
        if (!gTasks[taskId].tUsingRegisteredKeyItem)
            DisplayItemMessage(taskId, FONT_NORMAL, sText_JohtoRegionLocked, CloseItemMessage);
        else
            DisplayItemMessageOnField(taskId, sText_JohtoRegionLocked, Task_CloseCantUseKeyItemMessage);
    }
}
