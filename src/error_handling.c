...
u8 SafeGetRivalAverageLevel(u8 badge_count, u8 region_id, bool8 is_champion_slot)
{
    /*
     * Safe wrapper for GetRivalAverageLevel with validation.
     * is_champion_slot selects Champion formula vs. adaptive rival formula (Req 22).
     */
    if (!ValidateBadgeCount(badge_count))
        badge_count = 0;
    ...
    return ClampLevel(GetRivalAverageLevel(badge_count, region_id, is_champion_slot));
}

u8 SafeGetRivalAceLevel(u8 badge_count, u8 region_id, bool8 is_champion_slot)
{
    /*
    ...
    return ClampLevel(GetRivalAceLevel(badge_count, region_id, is_champion_slot));
}
...
bool8 ValidateStorageAccess(void)
{
    ...
    /* Additional validation: check party is not empty */
